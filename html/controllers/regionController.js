    ...
    $scope.oar = {
      ...
      save: function() {
        taskService.saveOar(this.current).then(
          function() {
            alertify.success("Save oar initiated for region " + $scope.oar.current.name);
            $scope.oar.modal.close();
          },
          function(msg) {
            alertify.error(msg);
          }
        ...
        alertify.confirm("Are you sure? This will irreversably wipe out any content you have in your region.", function(confirmed) {
          if (confirmed) {
            taskService.nukeRegion(region).then(
              function() {
                alertify.success("Nuke initiated for region " + region.name);
              },
              function(msg) {
                alertify.error(msg);
              }
      ...
      load: function(merge, x, y, z) {
        ...
        taskService.loadOar($scope.oar.current, data, merge, x, y, z).then(
          function() {
            alertify.success("Load oar initiated for region " + $scope.oar.current.name);
            $scope.oar.modal.close();
          },
          function(msg) {
            alertify.error(msg);
    ...
    $scope.region = {
      ...
      setHost: function(region, host) {
        ...
        regionService.setHost(region, host).then(
          function() {
            alertify.success("Region " + region.name + " moved to Host " + hostName);
          },
          function(msg) {
            alertify.error(msg);
          }
      ...
      setEstate: function(region, estate) {
        regionService.setEstate(region, estate).then(
          function() {
            alertify.success("Region " + region.name + " moved to Estate " + estate.name);
          },
          function(msg) {
            alertify.error(msg);
          }
      ...
      start: function(region) {
        regionService.start(region).then(
          function() {
            alertify.success("Region " + region.name + " signalled to start");
          },
          function(msg) {
            alertify.error(msg);
          }
      ...
      stop: function(region) {
        regionService.stop(region).then(
          function() {
            alertify.success("Region " + region.name + " signalled to stop");
          },
          function(msg) {
            alertify.error(msg);
          }
      ...
      kill: function(region) {
        regionService.kill(region).then(
          function() {
            alertify.success("Region " + region.name + " flagged for kill");
          },
          function(msg) {
            alertify.error(msg);
          }
        ...
        alertify.confirm("Do you really want to destroy " + region.name + " and all content?", function(confirmed) {
          if (confirmed) {
            regionService.remove(region).then(
              function() {
                alertify.success("Region " + region.name + " has been deleted");
              },
              function(msg) {
                alertify.error(msg);
              }
      ...
      setXY: function(region, x, y) {
        ...
        regionService.setXY(region, x, y).then(
          function() {
            alertify.success("position for region " + region.name + " changed successfully");
          },
          function(msg) {
            alertify.error(msg);
          }
      ...
      add: function(name, x, y, estate) {
        ...
        for (var i = 0; i < $scope.regions.length; i++) {
          if( $scope.regions[i].x === x && $scope.regions[i].y === y){
            alertify.error("Error, region " + $scope.regions[i].name + " is at those coordinates");
            return;
          }
        }
        regionService.add(name, x, y, estate).then(
          function() {
            alertify.success("Region " + name + " created");
            $scope.region.modal.close();
          },
          function(msg) {
            alertify.error(msg);
          }
